...
const backendUrl = (
  import.meta.env.VITE_AUTHFORGE_API_URL || "http://localhost:8082"
).replace(/\/$/, "");
...
function App() {
  ...
  const [machineToken, setMachineToken] = useState<MachineTokenResponse | null>(
    null,
  );
  ...
  const recordRequest = useCallback(
    <T,>(method: string, path: string, result: ApiResult<T>) => {
  ...
  const checkHealth = useCallback(async () => {
    ...
    setHealth(
      result.ok && result.data?.status?.toUpperCase() === "UP"
        ? "online"
        : "offline",
    );
  }, [recordRequest]);
  ...
  async function provisionClient(event: FormEvent) {
    ...
    if (!result.ok) {
      setNotice({
        kind: "error",
        message: errorMessage(result.data, result.status),
      });
      return;
    }
    ...
    setNotice({
      kind: "success",
      message: "Client provisioned — save the secret while it is visible",
    });
  }
  ...
  async function registerUser(event: FormEvent) {
    ...
    setNotice({
      ...
      message: result.ok
        ? result.data.message
        : errorMessage(result.data, result.status),
    });
  }
  ...
  async function loginUser(event: FormEvent) {
    ...
    setNotice({
      kind: "success",
      message: `Signed in as ${result.data.email || email}`,
    });
  }
  ...
  function startSocialLogin(provider: "google" | "github") {
    if (!clientId) {
      setNotice({
        kind: "error",
        message: "Enter or provision a client ID first",
      });
      return;
    }
    ...
    setNotice({
      kind: "info",
      message: `Opened ${provider} sign-in in a new tab`,
    });
  }
  ...
  async function exchangeSocialCode(event: FormEvent) {
    ...
    const result = await request<TokenResponse>(
      path,
      jsonRequest("POST", { code: exchangeCode }),
    );
    recordRequest("POST", path, result);
    setBusy(null);
    ...
    setNotice({
      kind: "success",
      message: "One-time code exchanged for user tokens",
    });
  }
    ...
    <div className="app-shell">
      <header className="topbar">
        <a
          className="brand"
          href="#top"
          aria-label="AuthForge Test Console home"
        >
        ...
        <section className="hero">
          <div>
            ...
            <h1>
              ...
              <br />
              <span>See every response.</span>
            </h1>
          </div>
          <div className="hero-note">
            <span className="hero-note-index">01—03</span>
            <p>
              Provision a tenant, authenticate a user, then inspect token claims
          ...
          <section className="workbench" aria-live="polite">
            {view === "client" && (
              <div className="view-stack">
                ...
                <form
                  className="form-grid"
                  onSubmit={(event) => void provisionClient(event)}
                >
                  ...
                  <div className="form-action wide">
                    <button
                      ...
                      type="submit"
                    >
                      {busy === "client" ? "Provisioning…" : "Provision client"}
                      <span>↗</span>
                    </button>
                  </div>
                </form>
                ...
                {client && (
                  <div className="credential-card">
                    ...
                    <SecretRow
                      ...
                      onCopy={() => copyToClipboard(client.clientId, setNotice)}
                    />
                    <SecretRow
                      label="Client secret"
                      value={client.clientSecret}
                      onCopy={() =>
                        copyToClipboard(client.clientSecret, setNotice)
                      }
                      sensitive
                    />
                  </div>
                ...
                <form
                  className="form-grid"
                  onSubmit={(event) => void requestMachineToken(event)}
                >
                  ...
                  <Field label="Requested scope" htmlFor="machine-scope" wide>
                    <input
                      id="machine-scope"
                      value={machineScope}
                      onChange={(event) => setMachineScope(event.target.value)}
                      required
                    />
                  </Field>
                  <div className="form-action wide">
                ...
                {machineToken && (
                  <TokenInspector
                    title="Machine access token"
                    token={machineToken.access_token}
                    claims={machineClaims}
                    onCopy={() =>
                      copyToClipboard(machineToken.access_token, setNotice)
                    }
                  />
                )}
              </div>
              ...
              <div className="view-stack">
                ...
                <div className="dual-actions">
                  <form
                    ...
                    onSubmit={(event) => void registerUser(event)}
                  >
                    <div className="action-card-number">A</div>
                    <h3>Create user</h3>
                    <p>
                      Add this identity to the active client with the default
                      user role.
                    </p>
                    ...
                    <button
                      ...
                      type="submit"
                    >
                      {busy === "register" ? "Registering…" : "Register user"}
                      <span>→</span>
                    </button>
                  </form>
                  <form
                    className="action-card dark"
                    onSubmit={(event) => void loginUser(event)}
                  >
                    <div className="action-card-number">B</div>
                    <h3>Sign in</h3>
                    <p>
                      Verify the credentials and capture the user access and
                      refresh tokens.
                    </p>
                    <div className="signin-summary">
                      <span>{email || "Email required"}</span>
                      <span>
                        {clientId ? compactToken(clientId) : "Client required"}
                      </span>
                    </div>
                    <button
                      ...
                      type="submit"
                    >
                      {busy === "login" ? "Signing in…" : "Sign in & inspect"}
                      <span>↗</span>
                    </button>
                  </form>
                </div>
                  ...
                  <>
                    <div className="token-actions">
                      ...
                      <button
                        ...
                        type="button"
                        disabled={busy !== null}
                        onClick={() => void validateUserToken()}
                      >
                      ...
                      <button
                        ...
                        type="button"
                        disabled={busy !== null}
                        onClick={() => void refreshUserToken()}
                      >
                        {busy === "refresh" ? "Rotating…" : "Rotate tokens"}
                      </button>
                      {validation !== null && (
                        <span
                          className={`validation-chip ${validation ? "valid" : "invalid"}`}
                        >
                    ...
                    <TokenInspector
                      ...
                      claims={userClaims}
                      onCopy={() =>
                        copyToClipboard(userToken.accessToken, setNotice)
                      }
                    />
                    <SecretRow
                      label="Refresh token"
                      ...
                      onCopy={() =>
                        copyToClipboard(userToken.refreshToken, setNotice)
                      }
                      sensitive
                    />
                  </>
            ...
            {view === "social" && (
              <div className="view-stack">
                ...
                <div className="flow-callout">
                  ...
                  <p>
                    The backend callback displays a short-lived code. Copy it,
                    return here, and exchange it within one minute.
                  </p>
                </div>
                <form
                  className="exchange-form"
                  onSubmit={(event) => void exchangeSocialCode(event)}
                >
                  <Field
                    label="One-time exchange code"
                  ...
                  >
                    <textarea
                      ...
                      rows={3}
                      placeholder="Paste the code returned by /auth/oauth2/callback"
                      required
                    />
                  </Field>
                  <button
            ...
            <div className="console-body">
              {activities.length === 0 ? (
                <div className="empty-console">
                  <span>_</span>
                  <p>API responses will appear here as you test each flow.</p>
                </div>
              ) : (
...
function TokenInspector({
  ...
  return (
    <div className="token-inspector">
      <div className="token-inspector-head">
        <span>{title}</span>
        <button type="button" onClick={onCopy}>
          Copy token
        </button>
      ...
      <div className="claims-head">
        <span>Decoded JWT payload</span>
        <small>Local decode · signature not verified here</small>
      </div>
      <pre>
        {claims
          ? formatJson(claims)
          : "Token is not a JWT or could not be decoded."}
      </pre>
    </div>
  );
}
  ...
  return (
    <details className="activity-item">
      <summary>
        ...
        <small>
          {activity.status || "ERR"} · {activity.durationMs}ms
        </small>
      </summary>
